{{--
    Contact Item Component - Linktree Style
    
    Modern Linktree-style contact button with gradient backgrounds,
    smooth animations, and prominent icon display.
    Used in contact page to render contact options dynamically.
--}}

@props([
    'icon' => '',
    'title' => '',
    'value' => '',
    'type' => 'default', // 'default', 'phone', 'email', 'whatsapp'
    'href' => '#',
])

@php
    $iconMap = [
        'phone' => 'M144.27,45.93a8,8,0,0,1,9.8-5.66,86.22,86.22,0,0,1,61.66,61.66,8,8,0,0,1-5.66,9.8A8.23,8.23,0,0,1,208,112a8,8,0,0,1-7.73-5.94,70.35,70.35,0,0,0-50.33-50.33A8,8,0,0,1,144.27,45.93Zm-2.33,41.8c13.79,3.68,22.65,12.54,26.33,26.33A8,8,0,0,0,176,120a8.23,8.23,0,0,0,2.07-.27,8,8,0,0,0,5.66-9.8c-5.12-19.16-18.5-32.54-37.66-37.66a8,8,0,1,0-4.13,15.46Zm81.94,95.35A56.26,56.26,0,0,1,168,232C88.6,232,24,167.4,24,88A56.26,56.26,0,0,1,72.92,32.12a16,16,0,0,1,16.62,9.52l21.12,47.15,0,.12A16,16,0,0,1,109.39,104c-.18.27-.37.52-.57.77L88,129.45c7.49,15.22,23.41,31,38.83,38.51l24.34-20.71a8.12,8.12,0,0,1,.75-.56,16,16,0,0,1,15.17-1.4l.13.06,47.11,21.11A16,16,0,0,1,223.88,183.08Zm-15.88-2s-.07,0-.11,0h0l-47-21.05-24.35,20.71a8.44,8.44,0,0,1-.74.56,16,16,0,0,1-15.75,1.14c-18.73-9.05-37.4-27.58-46.46-46.11a16,16,0,0,1,1-15.7,6.13,6.13,0,0,1,.57-.77L96,95.15l-21-47a.61.61,0,0,1,0-.12A40.2,40.2,0,0,0,40,88,128.14,128.14,0,0,0,168,216,40.21,40.21,0,0,0,208,181.07Z',
        'email' => 'M224,48H32a8,8,0,0,0-8,8V192a16,16,0,0,0,16,16H216a16,16,0,0,0,16-16V56A8,8,0,0,0,224,48Zm-96,85.15L52.57,64H203.43ZM98.71,128,40,181.81V74.19Zm11.84,10.85,12,11.05a8,8,0,0,0,10.82,0l12-11.05,58,53.15H52.57ZM157.29,128,216,74.18V181.82Z',
        'whatsapp' => 'M187.58,144.84l-32-16a8,8,0,0,0-8,.5l-14.69,9.8a40.55,40.55,0,0,1-16-16l9.8-14.69a8,8,0,0,0,.5-8l-16-32A8,8,0,0,0,104,64a40,40,0,0,0-40,40,88.1,88.1,0,0,0,88,88,40,40,0,0,0,40-40A8,8,0,0,0,187.58,144.84ZM152,176a72.08,72.08,0,0,1-72-72A24,24,0,0,1,99.29,80.46l11.48,23L101,118a8,8,0,0,0-.73,7.51,56.47,56.47,0,0,0,30.15,30.15A8,8,0,0,0,138,155l14.61-9.74,23,11.48A24,24,0,0,1,152,176ZM128,24A104,104,0,0,0,36.18,176.88L24.83,210.93a16,16,0,0,0,20.24,20.24l34.05-11.35A104,104,0,1,0,128,24Zm0,192a87.87,87.87,0,0,1-44.06-11.81,8,8,0,0,0-6.54-.67L40,216,52.47,178.6a8,8,0,0,0-.66-6.54A88,88,0,1,1,128,216Z',
        'instagram' => 'M227.36,28.78a13,13,0,0,0-7.1-7.1C215.72,20.54,204,22,128,22s-87.72-1.46-92.26.68A13,13,0,0,0,28.65,29.78C20.54,34.28,22,46,22,128s-1.46,87.72.65,92.26a13,13,0,0,0,7.1,7.1c4.54,2.13,15.25.68,92.26.68s87.72,1.46,92.26-.68a13,13,0,0,0,7.1-7.1c2.12-4.54.67-15.25.67-92.26S229.48,34.28,227.36,28.78ZM128,188A60,60,0,1,1,188,128,60,60,0,0,1,128,188Zm66-102a14,14,0,1,1,14-14A14,14,0,0,1,194,86Z',
        'tiktok' => 'M16.708 0.027c1.745-0.027 3.48-0.011 5.213-0.027 0.105 2.041 0.839 4.12 2.333 5.563 1.491 1.479 3.6 2.156 5.652 2.385v5.369c-1.923-0.063-3.855-0.463-5.6-1.291-0.76-0.344-1.468-0.787-2.161-1.24-0.009 3.896 0.016 7.787-0.025 11.667-0.104 1.864-0.719 3.719-1.803 5.255-1.744 2.557-4.771 4.224-7.88 4.276-1.907 0.109-3.812-0.411-5.437-1.369-2.693-1.588-4.588-4.495-4.864-7.615-0.032-0.667-0.043-1.333-0.016-1.984 0.24-2.537 1.495-4.964 3.443-6.615 2.208-1.923 5.301-2.839 8.197-2.297 0.027 1.975-0.052 3.948-0.052 5.923-1.323-0.428-2.869-0.308-4.025 0.495-0.844 0.547-1.485 1.385-1.819 2.333-0.276 0.676-0.197 1.427-0.181 2.145 0.317 2.188 2.421 4.027 4.667 3.828 1.489-0.016 2.916-0.88 3.692-2.145 0.251-0.443 0.532-0.896 0.547-1.417 0.131-2.385 0.079-4.76 0.095-7.145 0.011-5.375-0.016-10.735 0.025-16.093z',
        'maps' => 'M128,16a88.1,88.1,0,0,0-88,88c0,53.91,88,136,88,136s88-82.09,88-136A88.1,88.1,0,0,0,128,16Zm0,120a32,32,0,1,1,32-32A32,32,0,0,1,128,136Z',
    ];

    $gradientMap = [
        'phone' => 'from-blue-500 to-blue-600',
        'email' => 'from-red-500 to-red-600',
        'whatsapp' => 'from-green-500 to-green-600',
        'instagram' => 'from-pink-500 via-purple-500 to-indigo-500',
        'tiktok' => 'from-slate-900 to-slate-700',
        'maps' => 'from-orange-500 to-orange-600',
        'default' => 'from-indigo-500 to-purple-600',
    ];

    $borderGradient = 'from-slate-300 to-slate-400';

    // Generate href based on type if not provided
    $finalHref = $href;
    if ($href === '#') {
        switch ($type) {
            case 'phone':
                $finalHref = 'tel:' . preg_replace('/\D/', '', $value);
                break;
            case 'email':
                $finalHref = 'mailto:' . $value;
                break;
            case 'whatsapp':
                $finalHref = 'https://wa.me/' . preg_replace('/\D/', '', $value);
                break;
            case 'instagram':
                $finalHref = str_starts_with($value, 'http') ? $value : 'https://instagram.com/' . str_replace('@', '', $value);
                break;
            case 'facebook':
                $finalHref = str_starts_with($value, 'http') ? $value : 'https://facebook.com/' . str_replace('@', '', $value);
                break;
            case 'telegram':
                $finalHref = str_starts_with($value, 'http') ? $value : 'https://t.me/' . str_replace('@', '', $value);
                break;
            case 'tiktok':
                $finalHref = str_starts_with($value, 'http') ? $value : 'https://tiktok.com/@' . str_replace('@', '', $value);
                break;
            case 'maps':
                $finalHref = str_starts_with($value, 'http') ? $value : 'https://maps.google.com/?q=' . urlencode($value);
                break;
        }
    }

    $svgPath = $icon ?: ($iconMap[$type] ?? '');
    $gradient = $gradientMap[$type] ?? $gradientMap['default'];
@endphp

<a href="{{ $finalHref }}" target="_blank" rel="noopener noreferrer" class="group w-full">
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br {{ $borderGradient }} p-1 transform transition-all duration-300 hover:shadow-2xl hover:scale-105 active:shadow-2xl active:scale-105 focus-within:shadow-2xl focus-within:scale-105">
        <!-- Background glow effect -->
        <div class="absolute inset-0 bg-gradient-to-br {{ $borderGradient }} opacity-20 group-hover:opacity-100 active:opacity-100 blur-xl transition-opacity duration-300 ease-in-out animate-pulse"></div>
        
        <!-- Content container -->
        <div class="relative bg-white dark:bg-neutral-900 rounded-xl px-4 py-3 flex items-center gap-4 transition-all duration-300">
            <!-- Icon -->
            @if ($svgPath)
                @if (str_contains($svgPath, 'M16.708'))
                    <div class="flex-shrink-0 p-2 rounded-xl bg-gradient-to-br {{ $gradient }} text-white transform transition-transform duration-300 group-hover:scale-110 active:scale-110 focus-within:scale-110">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 34 34">
                            <path d="{{ $svgPath }}"></path>
                        </svg>
                    </div>
                @else
                    <div class="flex-shrink-0 p-2 rounded-xl bg-gradient-to-br {{ $gradient }} text-white transform transition-transform duration-300 group-hover:scale-110 active:scale-110 focus-within:scale-110">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 256 256">
                            <path d="{{ $svgPath }}"></path>
                        </svg>
                    </div>
                @endif
            @endif
            
            <!-- Text content -->
            <div class="flex-1 text-left">
                <h3 class="text-sm font-bold text-neutral-900 dark:text-white group-hover:font-extrabold transition-all duration-300">
                    {{ $title }}
                </h3>
            </div>
            
            <!-- Arrow indicator -->
            <div class="flex-shrink-0 text-neutral-400 group-hover:text-neutral-600 dark:group-hover:text-neutral-300 transform group-hover:translate-x-1 transition-all duration-300">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 256 256">
                    <path d="M181.66,133.66l-80,80a8,8,0,0,1-11.32-11.32L164.69,128,90.34,53.66a8,8,0,0,1,11.32-11.32l80,80A8,8,0,0,1,181.66,133.66Z"></path>
                </svg>
            </div>
        </div>
    </div>
</a>
