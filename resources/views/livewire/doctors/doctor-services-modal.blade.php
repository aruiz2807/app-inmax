<div>
    <x-ui.modal
    id="doctor-services-modal"
    animation="fade"
    width="2xl"
    heading="Incluir servicios"
    description="Seleccione los servicios que otorga esta medico/proveedor"
    x-on:close-doctor-services-modal.window="$data.close()"
    x-on:open-doctor-services-modal.window="$data.open()"
    >
                ...
                <table class="border-separate border-spacing-y-2">
                    ...
                    <tbody>
                        @foreach($doctorServices as $doctorService)
                            <tr wire:key="service-{{ $doctorService->id }}">
                                <td>
                                    <x-ui.text>
                                ...
                                <td>
                                    <x-ui.text>
                                        {{ $doctorService->service->type === 'Amount' ? 'Importe' : 'Evento' }}
                                    </x-ui.text>
                                </td>

                                <td class="pl-4">
                                    <x-ui.button wire:click="delete({{ $doctorService->id }})" type="button" icon="trash" variant="danger" size="sm"/>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
