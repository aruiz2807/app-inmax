<div>
    ...
    <div>
        <x-ui.card size="full">
            ...
            <p>Administre los planes para las membresías</p>
        </x-ui.card>
    </div>
    ...
    <x-ui.modal
        ...
        animation="fade"
        width="2xl"
        heading="{{$planId ? 'Editar plan' : 'Nuevo plan'}}"
        description="Ingrese la siguiente información para registrar un plan"
        x-on:close-plan-modal.window="$data.close()"
        x-on:open-plan-modal.window="$data.open()"
    >
        <form wire:submit="save">
            <x-ui.fieldset label="Información del plan">
                <x-ui.field required>
                    <x-ui.label>Nombre</x-ui.label>
                    <x-ui.input wire:model="form.name" name="name" placeholder="Plan basico" />
                ...
                <x-ui.field required>
                 ...
                 <x-ui.radio.group wire:model="form.type" name="type" label="Tipo de plan" variant="cards" direction="horizontal" class="mt-4">
                    <x-ui.radio.item
                        icon="user"
                        value="Individual"
                        label="Individual"
