<div x-data="{ customServiceModal: false }" @close-custom-service-modal.window="customServiceModal = false">
    ...
    <div class="grid grid-cols-[2rem_auto] justify-stretch items-center pt-4 pb-4">
        ...
        <x-ui.text class="text-2xl">{{ $isEditing ? 'Editar nota medica' : 'Nota medica' }}</x-ui.text>
    </div>
            ...
            @if(!empty($form->services[$service->id]))
                <div class="grid grid-cols-12 items-center gap-2 pb-2">
                    ...
                    <div class="col-span-4">
                        <x-ui.input
                            wire:model.live="form.servicePrices.{{ $service->id }}"
                            placeholder="0.00"
                            :disabled="$isEditing"
                        >
                            <x-slot name="prefix">$</x-slot>
                        </x-ui.input>
                        <x-ui.error name="form.servicePrices.{{ $service->id }}" />
                    </div>
                </div>
            @endif
        @endforeach
